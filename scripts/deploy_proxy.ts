...
async function start() {
  ...
  const sharedAddressPath = `${process.cwd()}/addresses/${
  ...
  const MarketFactory = ethers.getContractFactory('Market');
  const marketFactory = upgrades.deployProxy(MarketFactory, []);
